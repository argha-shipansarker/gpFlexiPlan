        ...
        <div className="flex flex-wrap">
            {
                value.map((value, index) => (
                    <button
                        className={`h-12 w-12 md:h-14 md:w-14 rounded-full border-1 flex justify-center items-center text-xxs md:text-xs ml-2 mb-2 md:ml-4 md:mb-3 font-telenor shadow-sm ${handleBackgroundColor(value)} ${manto(value)} ${value.selected ? "text-white" : "text-black"}`}
